...
public abstract class AbstractLengthPrependerClient extends AbstractClient
{
  ...
  {
    if (sendBuffer4Offers.capacity() - sendBuffer4Offers.size() < 3 && sendBuffer4Offers.capacity() == MAX_SENDBUFFER_SIZE) {
      return false;
    }
  ...
  {
    if (sendBuffer4Offers.capacity() - sendBuffer4Offers.size() < 2 && sendBuffer4Offers.capacity() == MAX_SENDBUFFER_SIZE) {
      return false;
    }
